...
export function ProfileClient({ student }: ProfileClientProps) {
    ...
    const getFeeStatusBadge = (status: StudentProfile['fees']['status']) => {
        switch(status) {
      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card className="glassmorphic">
             ...
             <Card className="glassmorphic">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><DollarSign className="h-5 w-5 text-primary" /> Fee Status</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="flex items-center justify-between mb-2">
                        {getFeeStatusBadge(student.fees.status)}
                        <span className="text-xl font-bold">${student.fees.due.toLocaleString()} Due</span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">Paid ${student.fees.paid.toLocaleString()} of ${student.fees.total.toLocaleString()}</p>
                </CardContent>
            </Card>
        </div>
        <Card className="glassmorphic">
